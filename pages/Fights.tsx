    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex bg-zinc-900 p-1 rounded-lg border border-zinc-800 overflow-x-auto">
          {(['ALL', FightStatus.UPCOMING, FightStatus.IN_PROGRESS, FightStatus.FINISHED] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
            ...
            >
              {f === FightStatus.IN_PROGRESS ? 'LIVE' : f}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex flex-col md:flex-row items-center animate-pulse">
               <div className="w-full md:w-32 h-24 bg-zinc-800 rounded-lg mb-4 md:mb-0 md:mr-8"></div>
               <div className="flex-grow w-full space-y-3">
                  <div className="h-8 w-3/4 bg-zinc-800 rounded mx-auto md:mx-0"></div>
                  <div className="flex justify-center md:justify-start gap-4">
                      <div className="h-4 w-24 bg-zinc-800 rounded"></div>
                      <div className="h-4 w-24 bg-zinc-800 rounded"></div>
                  </div>
               </div>
               <div className="mt-6 md:mt-0 w-full md:w-48 h-12 bg-zinc-800 rounded"></div>
            </div>
          ))
        ) : (
          filteredFights.map((fight) => (
            <Link
              ...
              className={`block bg-zinc-900 border rounded-xl overflow-hidden transition-all group ${
                  fight.status === FightStatus.IN_PROGRESS ? 'border-red-500 shadow-lg shadow-red-900/20' : 'border-zinc-800 hover:border-red-600'
              }`}
            >
              <div className="p-6 flex flex-col md:flex-row items-center">
                ...
                <div className="flex-shrink-0 w-full md:w-32 text-center mb-4 md:mb-0 md:mr-8 bg-zinc-950 rounded-lg py-4 border border-zinc-800 relative overflow-hidden">
                  ...
                  <span className={`block text-2xl font-black ${fight.status === FightStatus.IN_PROGRESS ? 'text-white' : 'text-red-500'}`}>
                ...
                <div className="mt-6 md:mt-0 flex items-center space-x-4 md:ml-8 min-w-[200px] justify-center">
                   <div className="text-right">
                      ...
                      <p className="text-xs text-zinc-500">
                        {fight.status === FightStatus.FINISHED && fight.resultWinnerId === fight.fighterAId ? 'WINNER' : ''}
                      </p>
                   </div>
                   ...
                   <div className="text-left">
                      <p className="font-bold text-white text-sm md:text-base">{fight.fighterB.name}</p>
                       <p className="text-xs text-zinc-500">
                        {fight.status === FightStatus.FINISHED && fight.resultWinnerId === fight.fighterBId ? 'WINNER' : ''}
                      </p>
                   </div>
                </div>
              </div>
